# Start Game Step 1 — Manager Information

> **Note:** This is the first of four internal Start Game setup steps and uses the shared structure defined in [global.md](global.md). Its controls intentionally match Favorite Team Step 1, but its initial values load from the persisted onboarding result into a separate game-setup draft. The interface must not show a step number, dots, boxes, counters, or progress bar.
...
| --- | --- | --- | --- |
| `ManagerNameInput` | Text input | Persisted manager name; fallback placeholder: **Manager Name** | Loads the manager name into the new-game draft and allows a draft-only edit |
| `CountrySelectField` | Read-only dropdown-style input | Persisted manager country flag and name; fallback placeholder: **Select Country** | Loads the manager nationality into the new-game draft and opens the country selection overlay |
...
When Start Game Step 1 first opens, initialize `ManagerInformationDraft` from the authoritative persisted Favorite Team onboarding result: manager name, manager country, coaching-style selection and values, and fluent-language selections. Do not default the country to Indonesia when a different persisted manager country exists, and do not create a second profile source of truth.

The user may edit these values for the new game. Keep every change inside the Start Game draft; continuing or completing this flow must not overwrite the persisted manager profile, favorite club, or original onboarding result. Returning Back within the Start Game flow preserves the draft. A later new Start Game session reloads the authoritative persisted profile unless a separate draft-resume policy is approved.

If the persisted profile is unavailable or incomplete, preserve the existing field placeholders and safe fallback rendering instead of showing `undefined`, inventing values, or silently mutating the main profile. Exact recovery copy and whether Start Game is blocked remain pending.
...
Keep `ManagerNameInput` and `CountrySelectField` side by side. Initialize them from the persisted manager name and country. `ManagerNameInput` retains **Manager Name** only as its unavailable-value placeholder; `CountrySelectField` retains **Select Country** as its fallback placeholder and opens the searchable country picker.

### Group 2 — Coaching and Language

Place `FieldGroupTwo` directly below Group 1 using the shared group gap. `CoachingStyleSelectField` visually matches the default input or button surface and initially displays the persisted coaching-style title and values. Use a global coaching icon beside the title. If no persisted style is available, display **Select**. After the user chooses a different template or creates a custom style for this game draft, show that title. Pressing the field pushes the full-screen `StartGameCoachingStyleScreen` route described below; it must not open a bottom overlay or replace content inside the Step 1 route.

`FluentLanguageSelectField` uses the same visual structure and interaction pattern as `CountrySelectField`: a `CountryFlagIcon` backed by the language record's explicit `displayCountryIsoCode`, selected language name, trailing dropdown chevron, searchable bottom overlay, sticky full-width search input, divider, scroll-edge fade, scrollable list, immediate selection, and automatic close. Initialize it from the persisted fluent-language selections; use **Select Language** only when no valid value exists or the user adds another language. Selecting a language updates only the Start Game draft. Country and language selection must share the reusable `SearchableFlagPickerSheet` primitives while supplying their own datasets, search labels, and row copy.

### Group 3 — Additional Fluent Languages

Place `FieldGroupThree` below Group 2. It contains `AddFluentLanguageButton`, rendered as the reusable icon-only **+** control with an accessibility label such as **Add fluent language**. Render persisted additional languages before this control. Pressing it inserts another `FluentLanguageSelectField` into the Start Game draft while preserving the same dropdown behavior. Keep the **+** control available until a product-defined maximum is reached. Do not invent the maximum, duplicate-language rules, or removal interaction yet.
...
`StartGameCoachingStyleScreen` is a separate full-screen Start Game route pushed onto the native stack from `CoachingStyleSelectField`. It is not the onboarding `CoachingStyleScreen`, a sheet, modal overlay, or conditional replacement inside `StepContent`. Pushing it preserves the Step 1 game draft beneath the route and gives forward and back navigation the standard native stack transition and interactive back behavior.
...
Use Expo Router or the project's native stack for the push and pop. Do not add a custom page animation over the native transition. Pressing the header Back control or using the platform back gesture pops to `StartGameManagerInformationStep`. If no style is selected, `CoachingStyleSelectField` displays **Select**. A predefined template displays its title and **Custom** displays **Custom**. Preserve every unrelated Start Game Step 1 draft value.
...
The global Back and Continue buttons remain supplied by `StepScreenLayout` and are not recreated in Step 1. **Back** returns to `HomeScreen` without committing draft-only changes. **Continue** pushes `StartGameCountrySelectionStep` after the Step 1 validation rules succeed and preserves the draft for Back navigation. Validation requirements and enabled or disabled behavior remain pending; do not infer required fields until validation rules are explicitly defined.

## Pending Decisions

Country flag rendering is fixed to the reusable `CountryFlagIcon` wrapper around `react-native-country-flag`. The remaining product decisions are the authoritative country and language datasets; each language's explicit `displayCountryIsoCode`; localization language; accessible flag fallback and offline Start Game requirements; picker empty states and search placeholders; sheet height and dismissal gestures; predefined coaching template names and attribute values; initial **Custom** values; template-to-custom transition behavior after manual edits; any total coaching-attribute point budget; maximum fluent-language entries; duplicate-language prevention and language-removal interaction; manager-name and field validation; missing-profile recovery; and exact route paths. The Back destination is fixed to `HomeScreen`, and Continue is fixed to `StartGameCountrySelectionStep`.
