# Start Game Step 3 — Select League

> **Note:** This is the third of four internal Start Game setup steps and uses the shared shell and synchronized selector contract in [global.md](global.md). Its selection UI remains identical to Favorite Team Step 3. Do not show a step number, dots, boxes, counters, progress bar, or other visible step indicator.
...
Only leagues belonging to `selectedCountryId` from Start Game Step 2 may appear. If the user returns to Step 2 and changes the country, rebuild the Step 3 dataset and clear any incompatible `selectedLeagueId` and team selection. Do not silently retain a league from another country.

## Navigation

The Step 3 **Back** action returns to `StartGameCountrySelectionStep` with the current country still centered and selected. **Continue** advances to `StartGameTeamSelectionStep` after a valid league has been selected because Step 4 team options depend on `selectedLeagueId`.
